...
//	pic_memfree(jpg_buffer);	//释放jpg_buffer申请到的内存
	SYS_FREE(f_jpeg,SRAMIN);
	SYS_FREE(jpeg_dev,SRAMIN);
	SYS_FREE(jpg_buffer,SRAMIN);
}
...
u8 whether_jpg(const u8 *filename)
{
  u8 res;
