...
{				
	...
	if((x+width-1)>LCD.width)return PIC_WINDOW_ERR;		//区域错误
	if((y+height-1)>LCD.height)return PIC_WINDOW_ERR;	//区域错误 
 	 
#if BMP_USE_MALLOC == 1	//使用malloc	
//	databuf=(u16*)pic_memalloc(2048);		//开辟至少bi4width大小的字节的内存区域 ,对240宽的屏,480个字节就够了.最大支持1024宽度的bmp编码
	SYS_MALLOC(databuf,SRAMIN,2048);
	if(databuf==NULL)return PIC_MEM_ERR;		//内存申请失败.
//	f_bmp=(FIL *)pic_memalloc(sizeof(FIL));	//开辟FIL字节的内存区域 
	SYS_MALLOC(f_bmp,SRAMIN,sizeof(FIL));
	if(f_bmp==NULL)								//内存申请失败.
...
//		pic_memfree(databuf);
 	...
 	if(res==FR_OK)//创建成功
	{
		...
		for(ty=y+height-1;hbmp.bmiHeader.biHeight;ty--)
		{
			...
			{
				if(pixcnt<hbmp.bmiHeader.biWidth)databuf[pixcnt]=LCD.ReadPoint(tx,ty);//读取坐标点的值 
