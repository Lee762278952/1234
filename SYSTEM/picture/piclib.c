...
{
	if((len==0)||(x0>LCD.width)||(y0>LCD.height))return;
	LCD.Fill(x0,y0,x0+len-1,y0,color);	
}
//填充颜色
//x,y:起始坐标
//width，height：宽度和高度。
...
{  
	u16 i,j;
	if(lcdltdc.pwidth!=0&&LCD.dir==0)//如果是RGB屏,且竖屏,则填充函数不可直接用
	{
		for(i=0;i<height;i++)
		{
			for(j=0;j<width;j++)
	...
	{
		LCD.Color_Fill(x,y,x+width-1,y+height-1,color);//其他情况,直接填充	
	}
}
//////////////////////////////////////////////////////////////////////////
...
{
	pic_phy.read_point=LCD.ReadPoint;  		//读点函数实现,仅BMP需要
	pic_phy.draw_point=LCD.Fast_DrawPoint;	//画点函数实现
	
	pic_phy.fill=LCD.Fill;					//填充函数实现,仅GIF需要
	pic_phy.draw_hline=piclib_draw_hline;  	//画线函数实现,仅GIF需要
	pic_phy.fillcolor=piclib_fill_color;  	//颜色填充函数实现,仅TJPGD需要 

	picinfo.lcdwidth=LCD.width;	//得到LCD的宽度像素
	picinfo.lcdheight=LCD.height;//得到LCD的高度像素
