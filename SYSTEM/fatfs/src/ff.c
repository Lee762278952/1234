...
#if _FS_EXFAT
#define	DEF_NAMBUF		BYTE sfn[12]; WCHAR *lfn = NULL;
...
#define INIT_NAMBUF(dj)	{ SYS_MALLOC(lfn,SRAMIN,((_MAX_LFN+1)*2 + SZDIRE*19)); if (!lfn) LEAVE_FF((dj).obj.fs, FR_NOT_ENOUGH_CORE); (dj).fn = sfn; (dj).lfn = lfn; (dj).obj.fs->dirbuf = (BYTE*)(lfn+_MAX_LFN+1); }
//#define	FREE_NAMBUF()	ff_memfree(lfn)
#define	FREE_NAMBUF()	SYS_FREE(lfn,SRAMIN)
...
#define INIT_DIRBUF(fs)	{ SYS_MALLOC(dirb,SRAMIN,(SZDIRE*19)); if (!dirb) LEAVE_FF(fs, FR_NOT_ENOUGH_CORE); fs->dirbuf = dirb; }
//#define	FREE_DIRBUF()	ff_memfree(dirb)
#define	FREE_DIRBUF()	SYS_FREE(dirb,SRAMIN)
...
#define INIT_NAMBUF(dj)	{ SYS_MALLOC(lfn,SRAMIN,((_MAX_LFN+1)*2));if (!lfn) LEAVE_FF((dj).obj.fs, FR_NOT_ENOUGH_CORE); (dj).fn = sfn; (dj).lfn = lfn; }
//#define	FREE_NAMBUF()	ff_memfree(lfn)
#define	FREE_NAMBUF()	SYS_FREE(lfn,SRAMIN)
