...
//		fs[i]=(FATFS*)mymalloc(SRAMIN,sizeof(FATFS));	//为磁盘i工作区申请内存	
		SYS_MALLOC(fs[i],SRAMIN,sizeof(FATFS));
		if(!fs[i])break;
	}
...
//	fatbuf=(u8*)mymalloc(SRAMIN,512);				//为fatbuf申请内存
	...
	SYS_MALLOC(file,SRAMIN,sizeof(FIL));
	SYS_MALLOC(ftemp,SRAMIN,sizeof(FIL));
	SYS_MALLOC(fatbuf,SRAMIN,512);
...
//	fbuf=(u8*)mymalloc(SRAMIN,8192);
	SYS_MALLOC(fsrc,SRAMIN,sizeof(FIL));
	SYS_MALLOC(fdst,SRAMIN,sizeof(FIL));
	SYS_MALLOC(fbuf,SRAMIN,8192);
...
//	myfree(SRAMIN,fdst);
//	myfree(SRAMIN,fbuf);
	SYS_FREE(fsrc,SRAMIN);
	SYS_FREE(fdst,SRAMIN);
	SYS_FREE(fbuf,SRAMIN);
	return res;
}
...
// 	finfo=(FILINFO*)mymalloc(SRAMIN,sizeof(FILINFO));
	SYS_MALLOC(fddir,SRAMIN,sizeof(DIR));
	SYS_MALLOC(finfo,SRAMIN,sizeof(FILINFO));
	
  if(fddir==NULL||finfo==NULL)res=100;
	if(res==0)
	{ 
// 		pathname=mymalloc(SRAMIN,MAX_PATHNAME_DEPTH);	 
		SYS_MALLOC(pathname,SRAMIN,MAX_PATHNAME_DEPTH);
...
//  			myfree(SRAMIN,pathname);	
				SYS_FREE(pathname,SRAMIN);				
		}
 	}
//	myfree(SRAMIN,fddir);    
//	myfree(SRAMIN,finfo);
	SYS_FREE(fddir,SRAMIN);
	SYS_FREE(finfo,SRAMIN);
...
//	finfo=(FILINFO*)mymalloc(SRAMIN,sizeof(FILINFO));
	SYS_MALLOC(srcdir,SRAMIN,sizeof(DIR));
	SYS_MALLOC(dstdir,SRAMIN,sizeof(DIR));
	SYS_MALLOC(finfo,SRAMIN,sizeof(FILINFO));
...
//		srcpathname=mymalloc(SRAMIN,MAX_PATHNAME_DEPTH);
		SYS_MALLOC(dstpathname,SRAMIN,MAX_PATHNAME_DEPTH);
		SYS_MALLOC(srcpathname,SRAMIN,MAX_PATHNAME_DEPTH);
...
// 			myfree(SRAMIN,srcpathname); 
				SYS_FREE(dstpathname,SRAMIN);
				SYS_FREE(srcpathname,SRAMIN);
		}
 	}
//	myfree(SRAMIN,srcdir);
//	myfree(SRAMIN,dstdir);
//	myfree(SRAMIN,finfo);
	SYS_FREE(srcdir,SRAMIN);
	SYS_FREE(dstdir,SRAMIN);
	SYS_FREE(finfo,SRAMIN);
