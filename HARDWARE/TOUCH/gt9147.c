	...
	if(strcmp((char*)temp,"9147")==0)//ID==9147
	{
		...
		if(temp[0]<0X60)//默认版本比较低,需要更新flash配置
		{
			printf("Default Ver:%d\r\n",temp[0]);
							if(LCD.id==0X5510)GT9147_Send_Cfg(1);//更新并保存配置
		}
		delay_ms(10);
		temp[0]=0X00;	 
		...
		if((mode&0XF)&&((mode&0XF)<6))
		{
			...
			for(i=0;i<5;i++)
			{
				if(tp_dev.sta&(1<<i))	//触摸有效?
				{
                    ...
                    if(LCD.id==0X5510)   //4.3寸800*480 MCU屏
                    {
                    ...
                    }else if(LCD.id==0X4342) //4.3寸480*272 RGB屏
                    {
                        if(tp_dev.touchtype&0X01)//横屏
                        {
                            tp_dev.x[i]=(((u16)buf[1]<<8)+buf[0]);
			...
			res=1;
			if(tp_dev.x[0]>LCD.width||tp_dev.y[0]>LCD.height)//非法数据(坐标超出了)
			{ 
